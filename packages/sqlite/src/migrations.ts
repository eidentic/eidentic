...
export const MIGRATIONS: ReadonlyArray<{ version: number; sql: string }> = [
  ...
  },
  {
    ...
    // We use a sidecar table (memory_meta) that stores ingested_at and metadata
    // keyed by (scope_key, ext_id), mirroring the approach used by blocks/block_history.
    // Old memories rows get no entry in memory_meta; the memory layer falls back to
    // in-memory map values or treats them as absent (similarity-only ranking, no metadata).
    sql: `
  ...
  },
  {
    ...
    // `last_corroborated_at` (epoch-ms) records the last re-confirmation; defaults to the fact's
    // validFrom at assert time. Both nullable — old rows read back as undefined.
    sql: `
  ...
  {
    version: 13,
    // Durable records need explicit ownership metadata so right-to-erasure can delete the exact
    // session/scope rows without parsing opaque idempotency keys. Backfill only when one existing
    // session is an unambiguous prefix; ambiguous legacy rows remain unowned rather than risking
...
export function runMigrations(db: Database.Database): void {
  db.exec(`CREATE TABLE IF NOT EXISTS _eidentic_migrations (version INTEGER PRIMARY KEY);`);
  const applied = new Set(
    db.prepare(`SELECT version FROM _eidentic_migrations`).all().map((r) => (r as { version: number }).version),
  );
  const tx = db.transaction((m: { version: number; sql: string }) => {
    db.exec(m.sql);
    db.prepare(`INSERT INTO _eidentic_migrations (version) VALUES (?)`).run(m.version);
  });
