...
/** The embedding-model type accepted by AI SDK v7 `embed`, minus the bare-string branch. */
type AIEmbeddingModel = Exclude<Parameters<typeof embed>[0]["model"], string>;

/**
 * Provider-agnostic hosted embedder over AI SDK v7. Bring your own provider + key + model:
...
export class AIEmbedder implements EmbeddingPort {
  ...
  /**
   * Batch embedding via AI SDK v7 `embedMany({ model, values })` → `{ embeddings: number[][] }`.
